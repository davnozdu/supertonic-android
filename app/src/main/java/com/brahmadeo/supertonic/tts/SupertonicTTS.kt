...
object SupertonicTTS {
    ...
    private external fun init(modelPath: String, libPath: String, ortThreads: Int): Long
    ...
    @Synchronized
    fun initialize(modelPath: String, libPath: String, ortThreads: Int = 5): Boolean {
        if (nativePtr != 0L) {
        ...
        nativePtr = init(modelPath, libPath, ortThreads)
        val success = nativePtr != 0L
        if (success) {
            Log.i("SupertonicTTS", "Engine initialized successfully (ORT: $ortThreads): $nativePtr")
        } else {
